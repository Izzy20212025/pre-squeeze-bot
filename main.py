...
# -------------------------
# Rate limiter (token bucket for UW)
# -------------------------
...
class UnusualWhales:
    def __init__(self, http: Http, cfg: Cfg, limiter: TokenBucket):
        ...
        self.headers = {
            ...
            "User-Agent": "pre-squeeze-scanner/2.0",
        }
...
async def sec_get_ticker_to_cik(http: Http, user_agent: str) -> Dict[str, str]:
    ...
    try:
        j = await http.get_json(url, headers=headers, ttl=7 * 24 * 3600, prefix="SEC_TICKERS")
        out = {}
        for rec in j.values():
...
async def sec_recent_filings_risk(http: Http, user_agent: str, cik10: str) -> Dict[str, Any]:
    ...
    headers = {"User-Agent": user_agent}
    try:
        j = await http.get_json(url, headers=headers, ttl=24 * 3600, prefix="SEC_FILINGS")
        ...
        for i, (form, dt) in enumerate(zip(forms[:50], dates[:50])):
            ...
            if "S-1" in form_upper or "S-3" in form_upper:
                risk += 15.0
                flags.append(f"{form}({dt})")
            elif "424B" in form_upper:
                risk += 10.0
                flags.append(f"{form}({dt})")
...
""".strip()
        ...
        try:
            async with self.s.post(url, headers=headers, json=payload, timeout=45) as r:
                ...
                self.cache.set("GROQ|" + ck, result, ttl=6 * 3600)
                return result
        except Exception:
            return {"sentiment": 0.0, "catalyst_strength": 0.0, "is_real_catalyst": False, "risk_flags": [], "summary": ""}
...
async def resolve_asof(poly: Polygon, asof_arg: Optional[str]) -> str:
    if asof_arg:
        return asof_arg
    d = date.today()
    for _ in range(10):
...
def presqueeze_score(feat: Dict[str, Any]) -> Tuple[float, Dict[str, float], str]:
    ...
    elif has_uw_short or (not math.isnan(finra) and finra >= 0.60 and vol_spike >= 2.0):
        bucket = "WATCHLIST"
    else:
    ...
    parts = {
        ...
        "E_options": E,
        "F_dilution": F,
        "pen_missing": missing_pen,
        "bonus_finra": finra_bonus,
        "bonus_combo": combo_bonus,
    }
        ...
        for c in cols:
            ...
            if c in ("ticker", "bucket"):
                ...
                if math.isnan(fv) or math.isinf(fv):
                    ...
                    if c in ("price",):
                        s = f"{fv:.2f}".ljust(18)
                    elif c in ("presqueeze_score", "rsi"):
                        s = f"{fv:.1f}".ljust(18)
...
async def main_async(args: argparse.Namespace) -> int:
    ...
    async with aiohttp.ClientSession(timeout=timeout) as session:
        ...
        print_table(f"🔥🔥 HIGH CONVICTION SQUEEZE CANDIDATES (asof={asof})", high_conv, top)
...
def build_argparser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(description="IMPROVED Pre-squeeze daily scanner with enhanced scoring")
